...
class PoorMansWordPressSEOMeta {
	
	const REMOVE_HEADERS_REGEXP = '#(<h([1-6])[^>]*>)\s?(.*)?\s?(<\/h\2>)#';
	const META_DESCRIPTION_MAX_LENGTH = 160;
	
	public function get_meta_title($title){
		$front_page_title_option = get_option('pm_wp_seo_frontpage_title');
		...
		global $post;
		$post_specific_title = get_post_meta($post->ID, 'pm_wp_seo_title', TRUE);
		
		if (!empty($post_specific_title)) {
			return $post_specific_title;
		}
		
		$use_automatic_titles = get_option('pm_wp_seo_use_automatic_titles') === PoorMansWordPressSEO::OPTION_ON;
		
		if (!$use_automatic_titles) {
			return $title;
		}
		
		$type_specific_title = get_option('pm_wp_seo_automatic_title_'.$post->post_type);
	...
	private function get_meta_description() {
		$front_page_description_option = get_option('pm_wp_seo_frontpage_description');
	...
	public function print_meta_opengraph() {
		...
		$facebook_app_id = get_option('pm_wp_seo_facebook_app_id', FALSE);
	...
	public function print_meta_noindex() {
		...
		if (is_tag()) {
			$noindex_for_tags = get_option('pm_wp_seo_noindex_for_tags');
			if ($noindex_for_tags === PoorMansWordPressSEO::OPTION_ON) {
				$print_noindex = TRUE;
			}
		}
		
		if (is_category()) {
			$noindex_for_categories = get_option('pm_wp_seo_noindex_for_categories');
			if ($noindex_for_categories === PoorMansWordPressSEO::OPTION_ON) {
				$print_noindex = TRUE;
			}			
		}
		
		if (is_date() || is_author()) {
			$noindex_for_archives = get_option('pm_wp_seo_noindex_for_archives');
			if ($noindex_for_archives === PoorMansWordPressSEO::OPTION_ON) {
				$print_noindex = TRUE;
			}				
		}
		
		if (is_paged()) {
			$noindex_for_paged_pages = get_option('pm_wp_seo_noindex_for_paged');
			if ($noindex_for_paged_pages === PoorMansWordPressSEO::OPTION_ON) {
				$print_noindex = TRUE;
			}			
		}
	...
	public function print_script_version() {
		echo "<!-- Poor Man's WordPress SEO " . PoorMansWordPressSEO::VERSION . " -->\n";
	}
	
	public function print_google_analytics_script() {
		$tracking_code = get_option('pm_wp_seo_google_analytics_code', FALSE);
