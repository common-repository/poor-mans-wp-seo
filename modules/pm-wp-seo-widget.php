...
class PoorMansWordPressSEOWidget {
	...
	public function print_dashboard_widget() {
		...
		echo '<tr><td><strong>'.__('Overall SEO score', PoorMansWordPressSEO::TEXT_DOMAIN).'</strong></td><td><strong>'.$this->get_overall_seo_status_score_text().'</strong></td></tr>';
		
		echo '<tr><td>'.__('Frontpage SEO score', PoorMansWordPressSEO::TEXT_DOMAIN).'</td><td>'.$this->get_frontpage_seo_status_text().'</td></tr>';
				
		echo '<tr><td>'.__('Articles SEO score', PoorMansWordPressSEO::TEXT_DOMAIN).'</td><td>'.$this->get_posts_seo_status_text('post').'</td></tr>';
		
		echo '<tr><td>'.__('Pages SEO score', PoorMansWordPressSEO::TEXT_DOMAIN).'</td><td>'.$this->get_posts_seo_status_text('page').'</td></tr>';
		
		echo '<tr><td>'.__('Sitemap updated', PoorMansWordPressSEO::TEXT_DOMAIN).'</td><td>'.$this->get_sitemap_status().'</td></tr>';
		
		echo '</table>';
		
		if ($indexing_denied) {
			printf('<table><tr><td class="icon"><span class="dashicons dashicons-warning"></span></td><td>'.__('Search engines are told not to index this site. Change the setting in <a href="%s">Reading</a> > Search Engine Visibility.', PoorMansWordPressSEO::TEXT_DOMAIN).'</td></tr></table>', get_admin_url().'/options-reading.php');
		}
	...
	private function get_overall_seo_status_score_text() {
		...
		$score_text = __('Low', PoorMansWordPressSEO::TEXT_DOMAIN) . ' <span class="dashicons dashicons-thumbs-down small-icon"></span>';
		
		if ($score > 0.9) {
			$score_text = __('High', PoorMansWordPressSEO::TEXT_DOMAIN) . ' <span class="dashicons dashicons-thumbs-up small-icon"></span>';
		}
		else if ($score > 0.6) {
			$score_text = __('OK', PoorMansWordPressSEO::TEXT_DOMAIN);
		}
	...
	private function get_frontpage_seo_status_score() {
		...
		$option_title = get_option('pm_wp_seo_frontpage_title');
		$option_description = get_option('pm_wp_seo_frontpage_description');		
	...
	private function get_posts_seo_status($post_type) {
		...
		while ($post_query->have_posts()) {
			...
			$seo_title = get_post_meta(get_the_ID(), 'pm_wp_seo_title', TRUE);
			$seo_description = get_post_meta(get_the_ID(), 'pm_wp_seo_description', TRUE);
	...
	private function is_excluded_post_id($id) {
		return in_array($id, explode(',', str_replace(' ', '', get_option('pm_wp_seo_exclude_posts'))));
	}
	...
	private function get_sitemap_status() {
		$settings_url = get_admin_url() . PoorMansWordPressSEO::ADMIN_SETTINGS_URL;
		
		$sitemap_enabled = get_option('pm_wp_seo_sitemap_enabled');
		$sitemap_updated = get_option('pm_wp_seo_sitemap_create_success', FALSE);
		$sitemap_created_time = get_option('pm_wp_seo_sitemap_create_time', FALSE);
	
		if ($sitemap_enabled !== PoorMansWordPressSEO::OPTION_ON) {
			return '<span class="pm-wp-seo-fail">'. sprintf(__('Not enabled. <a href="%s">Enable here</a>', PoorMansWordPressSEO::TEXT_DOMAIN), $settings_url) . '</span>';
		}
		
		if ($sitemap_updated === PoorMansWordPressSEO::STATUS_ERROR) {
			return '<span class="pm-wp-seo-fail">' . __('Error', PoorMansWordPressSEO::TEXT_DOMAIN) . '</span>';
		}
		
		if (($sitemap_updated === PoorMansWordPressSEO::STATUS_OK || $sitemap_updated === FALSE) && $sitemap_created_time !== FALSE) {
