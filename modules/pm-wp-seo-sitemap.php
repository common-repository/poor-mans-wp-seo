...
class PoorMansWordPressSEOSitemap {
	...
	public function __construct() {
		$this->imageSitemap = new PoorMansWordPressSEOImageSitemap();
		$this->pinger = new PoorMansWordPressSEOPing();
	}
	
	public function create_sitemap_from_control_panel() {
		$sitemap_enabled = get_option('pm_wp_seo_sitemap_enabled') === PoorMansWordPressSEO::OPTION_ON;
		
		if (!$sitemap_enabled) {
			update_option('pm_wp_seo_sitemap_create_success', 'not_enabled');
		...
		if ($sitemap_success) {
			update_option('pm_wp_seo_sitemap_create_time', time());
			update_option('pm_wp_seo_sitemap_create_success', PoorMansWordPressSEO::STATUS_OK);
			$this->pinger->ping_search_engines();
		}
		else {
			update_option('pm_wp_seo_sitemap_create_success', PoorMansWordPressSEO::STATUS_ERROR);
		}
		
		if ($image_sitemap_success) {
			update_option('pm_wp_seo_image_sitemap_create_time', time());
			update_option('pm_wp_seo_image_sitemap_create_success', PoorMansWordPressSEO::STATUS_OK);
		}
		else {
			update_option('pm_wp_seo_image_sitemap_create_success', PoorMansWordPressSEO::STATUS_ERROR);
		}
	...
	private function generate_sitemap_xml() {
		$option_priorities = get_option('pm_wp_seo_sitemap_prioritities');
		$option_include_lastmod = get_option('pm_wp_seo_sitemap_include_lastmod') === PoorMansWordPressSEO::OPTION_ON;
		$option_include_tags = get_option('pm_wp_seo_sitemap_include_tags') === PoorMansWordPressSEO::OPTION_ON;
		$option_include_categories = get_option('pm_wp_seo_sitemap_include_categories') === PoorMansWordPressSEO::OPTION_ON;
